...
**Import paths in examples:** Snippets embedded in guides and API pages often use `import … from './src/index.js'` as if you are developing **inside this repository**. When you consume the **`kviews`** package from npm, use `import … from 'kviews'` instead (typically resolved via your bundler to `dist/index.js`).
...
After `npm install kviews handlebars jquery`:
...
Use a bundler so the `kviews` import resolves (`dist/index.js` is the package entry).
...
Your bundler must resolve `kviews` to `node_modules/kviews/dist/index.js`.
